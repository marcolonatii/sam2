...
class SAM2CameraPredictor(SAM2Base):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.frame_idx = 0
        self.resizer = torchvision.transforms.Resize((self.image_size, self.image_size))
    ...
    def perpare_data(
        ...
        assert B == 1, "input image should be a single image"

        img = img.squeeze(0)  # remove batch dimension
        img = self.resizer(img)
        img = img / 255.0

        img_mean = torch.tensor(img_mean, dtype=torch.float32)[:, None, None].to(
            img.device
        )
        img_std = torch.tensor(img_std, dtype=torch.float32)[:, None, None].to(
            img.device
        )
        img -= img_mean
        img /= img_std
    ...
    def load_first_frame(self, img):
        ...
        img, width, height = self.perpare_data(img)
    ...
    def add_new_prompt(
        ...
        assert bbox is not None or points is not None, (
