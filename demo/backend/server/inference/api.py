...
from inference.data_types import (
    AddPointsRequest, CancelPorpagateResponse, CancelPropagateInVideoRequest,
    ...
    StartSessionRequest, StartSessionResponse, AddMaskRequest
)
...
class InferenceAPI:
    ...
    Persists user clicks and masks to disk in the SESSIONS_PATH directory.
    """
    ...
    def start_session(self, request: StartSessionRequest) -> StartSessionResponse:
        ...
        Loads persisted state if available, otherwise initializes a new state.
        ...
        with self.inference_lock:
            ...
            if session_id in self.session_states:
                ...
                if "state" in session:
                    session["state"]["last_use_time"] = time.time()
                     ...
                     # Re-initialize or handle error? For now, just return existing ID.
            ...
            try:
                ...
                raw_frames = video_reader.read_raw_frames(request.path, DATA_PATH)
                num_frames_loaded = len(raw_frames)
                if num_frames_loaded == 0:
                    logger.warning(f"Video {request.path} loaded 0 frames.")
                    # Decide how to handle: raise error or allow empty session?
                    # Let's allow empty session for now, download ops might fail later.

                # Initialize predictor state
                offload_video_to_cpu = self.device.type == "mps" # Specific handling for MPS
                absolute_video_path = str(DATA_PATH / request.path) # Predictor expects absolute path
                ...
                if session_dir.exists():
                    ...
                    # Store names in the inference state if needed elsewhere, or just load on demand
                    inference_state["object_names"] = loaded_names # Example storage
                ...
                # Store the initialized state in the session manager
                ...
                raise # Re-raise specific error for Flask handler
    ...
    def close_session(self, request: CloseSessionRequest) -> CloseSessionResponse:
        """
        Close an inference session, saving its state to disk before removing from memory.
        ...
        try:
            if session_id in self.session_states:
                ...
                success = False # Or True if "already closed" is considered success?
    ...
    def remove_object(self, request: RemoveObjectRequest) -> RemoveObjectResponse:
        """
        Remove an object from the segmentation state, update masks, and persist.
    ...
    def cancel_propagate_in_video(self, request: CancelPropagateInVideoRequest) -> CancelPorpagateResponse:
        ...
        with self.inference_lock:
            ...
            if request.session_id not in self.session_states:
                 ...
                 # Should this be success=False? Or is cancelling a non-existent task "successful"?
                 # Let's return success=False as the intended action (cancelling) didn't apply.
    ...
    def download_masks(self, request: DownloadMasksRequest) -> DownloadMasksResponse:
        ...
        This method provides the raw RLE mask data, suitable for JSON output.
        ...
        with torch.no_grad(), self.autocast_context(), self.inference_lock:
            ...
            # Delegate to downloaders module - uses cache or predictor.propagate_in_video
    ...
    def list_sessions(self) -> List[SessionInfo]:
        ...
        with self.inference_lock: # Lock needed to safely iterate over session_states
            ...
            for session_id, session_data in self.session_states.items():
                ...
                # Calculate num_objects based on the standard structure used by predictor
                # obj_ids = inference_state.get("obj_ids", []) # Get list of active object IDs from predictor state?
                # Or use the keys from points/labels? Let's use points keys as proxy
                obj_ids = set()
